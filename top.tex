...
\end{df}
\begin{df} Für $\omega\in \Omega(X; x_0, x_1)$ und $\eta\in \Omega(X; x_1, x_2)$ sei das \emph{Produkt} $\omega*\eta$ definiert durch:
 \[
  \omega*\eta(t):=\begin{cases} \omega(2t) &\text{ falls } t\in[0,\frac{1}{2})\\ \eta(2t-1) &\text{ falls } t\in[\frac{1}{2}, 1] \end{cases}
 \]
\begin{figure}[H]
\centering
 \fixme[fig49]
...
\caption{}
\end{figure}
 Seien $\omega, \eta \in \Omega(X; x_0, x_1)$. Eine \emph{Homotopie mit festen Endpunkten} von $\omega$ nach $\eta$ ist eine stetige Abbildung
\[
 H: [0,1]\times [0,1] \to X,
\]
so dass gilt: 
\begin{align*}
 ...
 H(t,1)=\eta(t)&, H(1,s)=x_1
\end{align*}
\end{df}
\begin{lem}
 Die Relation $\omega\sim \eta:\iff$ (es gibt eine Homotopie mit festen Endpunkten von $\omega$ nach $\eta$) ist eine Äquivalenzrelation.
\end{lem}
\begin{proof}
...
\begin{enumerate}[(i)]
 \item $\omega \sim \omega' \land \eta \sim \eta' \implies \omega *\eta \sim \omega' * \eta'$
  \item $\epsilon * \omega \sim \omega \sim \omega* \epsilon$ 
\item $\omega * \omega^{-1} \sim \epsilon \sim \omega^{-1}*\omega$
 \item $(\omega * \eta)*\xi \sim \omega*(\eta * \xi)$
\end{enumerate}
...
Sei $f$ die Homotopie zwischen $\omega$ und $\omega'$ und $g$ die Homotopie zwischen $\eta$ und $ \eta'$. Definiere:
\[
 H:[0,1]\times[0,1]\to X
\]
\[
...
\begin{st}
 Das Produkt von Schleifen $(\omega, \eta) \mapsto \omega * \eta$ induziert ein Produkt auf $\pi_1(X;x_0)$. Für 
