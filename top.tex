...
\begin{note*}
\begin{enumerate}[(\roman{enumi})]
\item Natürlich ist die Definition so gemacht, dass die in einem Topologie bilden. \fixme[nachschauen]
...
wäre $z\in B_\eps \land B_\eps(y)$,        
\end{proof}
\begin{st}
 Jeder Teilraum eines Hausdorffraums ist Hausdorffsch, Zwei topologische Räume sind genau dann Hausdorffsch, wenn es ihre topologische Summe ist und zwei nichtleere topologische Räume ist und zwei nichtleere topologische Räume sind Hausdorffsch, genau dann wenn es ihr Produkt ist.
\end{st}
\begin{proof}
 zur Übung
\end{proof}
Ein Raum der nicht Hausdorffsch ist, scheint mit unserer Intuition nicht vereinbar zu sein. EIn triviales Beispiel ist die Klumpentopologie $\{\empty, X\}$ auf jeder Menge. Mit Hilfe
...
Die Abbildung $\phi: \R\to Y, t\to e^{i2\pi t}$ (bzw. $(\cos(2\pi t), \sin(2\pi t))^T$) ist stetig, also auch $\bar \phi: \R/\sim \to Y, [t]\to e^{i2\pi t}$, da $\bar \phi$ bijektiv ist, folgt, daraus $\bar \phi$ ein Homöomorphismus ist, insbesondere gilt $\R/\sim \stackrel{\sim}{=}S^1$
