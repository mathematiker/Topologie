...
\end{st}
\begin{proof}
Sei $f:X\to Y$ stetig und $U_i, i\in I$ eine offene Überdeckung von $Y$. Dann ist $f^{-1}(U_i),i\in I$, eine offene Überdeckung von $X$. Diese besitzt aber eine endliche Teilüberdeckung $f^{-1}(U_{i_1})\cup ...\cup f^{-1}(U_{i_n})=X$. Dann ist $U_{i_1}\cup ...\cup U_{i_n}$ eine offene Überdeckung von $Y$, falls $f$ surjektiv ist.
\end{proof}
...
Der Schnitt aller $V_a, a\in A$ ist möglicherweise nicht offen (da eventuell Schnittmenge unendlich vieler Mengen), aber da wir wegen der Kompaktheit von $A$ endlich viele Mengen $U_{a_1},..., U_{a_n}$ auswählen können, so dass $A\subset U_{a_1}\cup...\cup U_{a_n}$. Dann gilt: $V_{a_1}\cap...\cap V_{a_n}$ ist offen und liegt ganz im Komplement von $A$.
\end{proof}
...
nn sie zu einer konstanten Abbildung homotop ist.  
\fixme[from mathematiker: korrigieren]
\end{enumerate}
\end{exs*}
\begin{df}
...
\makebox{
\begin{xy}
 (0,0)*+{S^n\times [0,1]}; (40,0)*+{\overline{B_1(0)}}; (20, -20)*+{S^n\times [0,1]/\sim};
...
\begin{exs*}
 \begin{enumerate}
  ...
  \setcounter{figure}{43}  %%% entfernen, wenn alle Grafiken drin sind :)
\begin{figure}[H]
\centering
 \fixme[fig44]
\caption{}
...
\begin{exs*}
 ...
 \end{enumerate}
 \fixme[from mathematiker : Scheint unvollständig zu sein]
...
\centering
 ...
 \caption{}
\end{figure}
 wegzusammenhängend $\nimplies$ lokal wegzusammenhängend. Gegenbeispiel: Vereinigung von Geradenstücken in $\R^2$:\\
\begin{figure}[H]
\centering
...
lokal wegzusammenhängend $\nimplies$ wegzusammenhängend. Gegenbeispiel: Zweipunktmenge.
\end{note*}
...
Wir erinnern: die induzierten Homomorphismen auf der Fundamentalgruppen sind:
\[
 f: X\stackrel{\text{stetig}}{\Longrightarrow} Y: x\mapsto y \longrightarrow \pi_1(X;x_0)
\]
induziert:
\[
...
\end{kor}
\begin{note*}
 den Urbildbereich einer Überlagerung ezeichnet man als \emph{Totalraum}. Und den Bildbereich \emph{Basisraum} $B$. 
\end{note*}
...
\end{df}
Wir wollen den Hochhebungssatz nun benutzen, wenn eine Wirkung einer gewissen Untergruppe der Fundamentalgruppe der Basis auf den Totalraum zu definieren. Es handelt sich dabei um die Fortsetzung der in Satz \ref{thm2:3.8} definierten Wirkung auf der Faser $E_b$.
...
\[
 \pi_1(\pi_1(E;e'_0))=[\omega]^{-1}\cdot \pi_1(p)(\pi_1(E;e_0)\cdot [\omega]
\]
\begin{figure}[H]
\centering
 \fixme[fig91]
...
\begin{note*}
Kunjugation von $a$ mit $g$ ist $gag^{-1}$
\end{note*}
...
\]
aller Elemente, die $H$ normalisieren heißt \emph{Normalisator} von $H$. Falls $N_G(H)=G$, dass heißt galls $gHg^{-1}=H \, \forall g\in G$, dann sagt man, $H$ ist eine \emph{normale Untergruppe}. Für solche gilt, dass die Menge der Nebenklasse $G/H:=\{gH|g\in G\}$ eine natürliche Gruppenstruktur hat $(g\cdot H)cdot (k\cdot H)=g k \underbrace{(k^{-1}Hk)}_{=H}H=g\cdot k \cdot HH=g\cdot $.
...
Dann operiert die diskrete Gruppe $N(p;e_0)/H(p;e_0)$ blätternd von rechts auf $E$, so dass die von den Gruppenelementen induzierten Homoöomorphismen HOchhebungen von $p:E\to B$ sind.
...
\caption{}
\end{figure}
 Zunächst ist klar, dass die oben definierten stetigen Abbildungen $E\to E$ eine stetige Fortsetzungen der Wirkung der 
Fundamentalgruppe $\pi_1(B;b_0)$ auf dr Faser über $b_0$ sind (vgl. Satz \ref{thm2:3.8}) auf ganz $E$ sind.  Diese 
Abbildung sind also nach dem Hochhebungssatz bereits eindeutig durch das Bild des Puntkes $e_0$ bestimmt. 
Daraus folgt, dass es sich um eine Gruppenwirkung handelt. Die Untergruppe $H(p, e_0)$ ist dabei 
gerade die Standgruppe von $e_0$ diese Elemente werden trivial auf $E$.

Da die von dieser Gruppenwirkung induzierte Homöomorphismen von $E$ mit $p$ verträglich sind, wrken sie auf dem Urbild $p^{-1}(U)=\dotcup_{\alpha} U_\alpha$ einer trivial überlagerten Umgebung $U$ durch Permutation der Blätter $U_\alpha$, also ist die Gruppenwirkung blätternd.
\begin{figure}[H]
\centering
 \fixme[fig94]
\caption{}
\end{figure}
Also ist nach Lemma \ref{thm2:3.5} die Abbildung $\bar{p}$ eine Überlagerung. Die Aussage über das Bild von $\pi_1(\bar p)$ ergibt sich aus Satz \ref{thm2:3.8}, dem $K$ ist die Standgruppe der $\pi_1(B;b_0)$-Wirkung auf der Faser $\bar p^{-1}(b_0)$ in diesem Fall.
\end{proof}
Mit Hilfe von Satz \ref{thm2:3.14} können wir nun \emph{alle} Überlagerungen des Raumes $B$ konstruieren, sobald wir eine universelle (=einfach zusammenhängende) Überlagerung $\tilde p: \tilde B\to B$ gefunden haben, denn für diese gilt $H(\tilde p; e_0)=\{1\}$ und $N(\tilde; e_0)=\pi_1(B;b_0)$, dass heißt wir können damit zu jeder Untergruppe $K\subset \pi_1(B;b_0)$ eine "'passende"' Überlagerung $p:E\to B$ mit $H(p;e_0)=K$ konstruieren.
Aus Korrolar \label{thm2:3.12} (Vergleich von Überlagerungen) folgt nämlich insbesondere, dass zwei Überlagerungen $p_1, p_2: E_1, E_2 \to B$ mit $H(p_1;e_0)=H(p_2;e_0)$ \emph{isomorph} sind, dass heißt es gibt einen Homöomorphismus $f:E_1\to E_2$ mit $p_2\circ f = p_1$.

Die Existenz einer universellen Überlagerung werden wir im nächsten Abschnitten beweisen (unter gewissen Voraussetzungen).
...
\begin{df}
 Sei $p:E\to B$ eine Überlagerung. Ein Homöomorphismus $f:E\to E$ heißt \emph{Decktransformation} oder \emph{Automorphismus} der Überlagerung $p$, falls $p\cdot p \cdot f=p$.
\end{df}
...
\begin{st}
 Sei $p:E\to B$ eine universellle Überlagerung. Dann gilt $\Deck(p)\cong \pi_1(B)$, falls $B$ wegzusammenhängend und lokal wegzusammenhängend ist.
...
\end{seg}
\begin{proof}
 Nach Satz \ref{thm2:3.14} gilt bei universellen Überlagerungen, dass die Gruppe $N(p; e_0)/H(p;e_0)=\pi_1(B;b_0)\setminus\{1\}=\pi_1(B;b_0)$ blätternd auf $E$ durch Decktransformationen wirkt. Insbesonders wirkt nun die Identität trivial und wir erhalten einen injektiven Homomorphismus:
\[
 \jota: \pi_1(B;b_0)\to \Deck(p)
\]
Es bleibt, Surjektivität zu zeigen.  Sei also $\Gamma\in \Deck(p)$. Sei $\omega$ ein Weg von $e_0$ nach $e_0\circ \gamma$. Dann ist $p\circ \omega$ eine Schleife mit Basispunkt $b_0$. Dies liefert eine Abbildung $f: \Deck(p)\to \pi_1(B,b_0)$, denn: ist $\eta$ ein zweiter Weg von $e_0$ noch $e_0 \circ \gamma$.
\begin{figure}[H]
\centering
 \fixme[fig97]
\caption{}
\end{figure}
so ist $\eta$ zu $w$ homotop mit festen Endpunkten, wegen des einfachen Zusammenhangs von $E$. Es gilt $\jota\circ f = \id_{\Deck(p)}$.

Denn der Homöomorphismus $\jota(f(\gamma))$ bildet $e_0$ auf $e_0\circ \gamma$ ab, und wir schließen daraus, dass $\jota(f(\gamma))=\gamma$, denn sonst gäbe es neben der Identität auf eine zweite Hochhebung von $p$, nämlich $\eta(f(\gamma))\gamma^{-1}$, die $e_0 \circ \gamma$ auf sich abbildet.
\end{proof}
\begin{kor}
...
\end{kor}
\begin{proof}
 Offensichtlich sind die Elemente von $\Gamma$ Decktransformationen von $X$, denn die Bahnen der $\Gamma$-Wirkung sind die Fasern der Überlagerung, also $\Gamma\subset \Deck(p)$. Sei $e\in X$. Es gilt also zu jedem Element der Faser $e'\in E_{p(e)}=e\cdot \Gamma$ ein $\gamma \in \Gamma$ mit $e\cdot \gamma=e'$, da die Decktransformation durch das Bild eines Punktes eindeutig bestimmt sind, kann es also nicht mehr Element in $\Deck(p)$ geben, die nicht in $\Gamma$ liegen. 
\end{proof}
...
\end{note*}
\begin{ex*}["`Hawaiianischen  Ohrringen"']
 \begin{figure}[H]
\centering
 \fixme[fig98]
\caption{}
\end{figure}
Diese ist nur nicht lokal einfach zusammenhängend und damit auch nicht semi-lokal einfach zusammenhängend.

Für einen Raum, der semi-lokal einfach zusammenhängend, aber nicht lokal zusammenhängend ist der "`Kegel über den Hawaiianischen Ohrringen"'
\begin{figure}[H]
\centering
 \fixme[fig99]
\caption{}
\end{figure}
Dieser ist semi-lokal, aber nicht lokal einfach zusammenhängend. Diese Eigenschaft ist notwendig für die Existenz einer universellen Überlaterung $p: E \to B$;
\begin{figure}[H]
\centering
\fixme[fig100]
\caption{}
...
Sei $b_0\in B$ ein fest gewählter Basispunkt und $b_1$ ein weiterer Punkt in $B$, wenn $w$ ein Weg von $b_0$ nach $b_1$ ist, dann erhalten wir mit der im Abschnitt 3 eingeführter Abbildung $\Lambda_\omega$ eine Abbildung von der Faser $E_{b_0}$ in die Faser $E_{b_1}$. Diese hängt nur von der Homotopieklasse (mit festen Endpunkten) ab.\\
\begin{figure}[H]
 \centering
\fixme[fig101]
\caption{}
\end{figure}
Ist andereseits $\Lambda_{\omega}(e)=\Lambda_{\eta}(e)$ für anderen Weg $\eta$ von $b_0$ nach $b_1$, dann ist $\eta$ auch homotop (m.f.E.\footnote{mit festen Endpunkten}) zu $\omega$, denn die Hochhebungen von $\omega$ und $\eta$ lassen sich ineinander deformieren, da $E$ einfach zusammenhängend ist.
...
\begin{st}[Existenz der universellen Überlagerung]
 Sei $B$ ein wegzusaamenhängender, lokal wegzusammenhängender und semi-lokal einfach zusammenhängender topologischer Raum. Dann existiert eine universelle Überlagerung $p:E \to B$
\end{st}
\begin{proof}
 Sei $b_0\in B$. Sei 
\[
...
\]
Es folgt, da $\eta$ zu $\eta'$ homotop (m. f. E.) ist, dass die Abbildung $\Phi$ wohldefiniert. (hier geht der semi-lokale einfache Zusammenhang ein!)
...
Die Mengen $V(U, \alpha)$ überdecken ganz $E$. Ein Durchschnitt zweier slcher Mengen ist entweder leer oder die Wegzusammenhangskomponenten sind entweder leer oder von der Form $V(U', \alpha')$.

Daher gibt es (genau) eine Topologie, dür die die $V(U, \alpha)$ offen sind (dass heißt verwenden die Mengen $V(U, \alpha)$ als Basis) und diese induziert auf den $V(U, \alpha)$ die oben gewählte. Damit ist $p:E \to B$ eine Überlagerung. Nach der Vorbemerkung und Korollar \ref{thm2:3.9} folgt, dass $E$ einfach zusammenhängend ist.
\begin{figure}[H]
\centering
 \fixme[fig104]
\caption{}
...
beim Satz von Seifert und van Kampen handelt es sich um ein starkes Hilfsmittel zur Berechnung von Fundamentalgruppen. Wenn ein topologischer Raum $X$ die Vereinigung affiner Teilmengen $U,V$ ist, so dass $U,V$, und $U\cap V$ wegzusammenhängend sind, dann gibt der Satz an, wie sich die Fundamentalgruppe von $X$ aus denen von $U$ und $V$ "`zusammengesetzt"'.
\begin{figure}[H]
 \centering
 \fixme[fig105]
\caption{}
...
\]
Eine Untergruppe $H\subset G$ ist eine Normalteiler genau dann, wenn $H$ als Kern eines Gruppenhomomorphismus auftritt, dass heißt falls es einen Homomorphismus $\phi: G\to K$ gibt mit $\ker \phi=H$.

Denn: ist $H$ ein Normalteiler, so ist $H$ der Kern des (surjektiven) Gruppenhomomorphisismum $\pi: G \to G/H: g\mapsto gH$ (wobei $G/H$ die natürliche Gruppenstruktur $(g_1\cdot H)\cdot (g_2 H)=g_1 g_2 H$ trägt, siehe (Abschnitt 3). Ist andererseits $\phi:G\to K$ ein Gruppenhomomorphismus und $h\in \ker \phi$, so liegt auch $ghg^{-1}$ im Kern, denn:
\[
 \phi(g\cdot h \cdot g^{-1})= \phi(g)\cdot\underbrace{\phi(h)}_{=1} \cdot \phi(g)^{-1}=1
\]
(Bei abelschen Gruppen sind alle UNtergruppen Normalerteiler, zum Beispiel $2\Z \subset \Z \to \Z^2=\Z_2=\Z/2\Z$)
Stets sind $\{1\}, G\subset G$ Normalteiler. Bezeichnung  $H\trianglelefteq G$ bedeutet $H\subset G$ Normalteiler)

Das \emph{direkte} Produkt $G_1\times... \times G_k$ von $k$ Gruppen $G_1, ..., G_k$ ist die Menge:
\[
 \{(g_1, g_2,...,g_k)|g_i \in G_i\}
\]
mit der komponentenweisen Verknüpfung
\[
 (g_1,...,g_k)\cdot (h_1,..., h_k):=(g_1h_1,..., g_kh_k)
\]
als Gruppenstruktur. Die Faktoren $G_i$ sind auf kanonische Weise durch die injektive $G_i\to G_1\times ... \times G_k, g_i \mapsto (1,..., 1, g_i, 1,..., 1)$ eingebettet (als $\{1\}\times... \times\{1\} \times G_i \times\{1\}  ... \times \{1\})$) und kommutieren miteinander, sind insbesondere Normalteiler.

 Eine ganz andere Konstruktion ist das \emph{freie Produkt} von Gruppen. Seien $G_1, ..., G_k$ disjunkte Gruppen. Ein \emph{Wort} der \emph{Länge} $l\in \N_0$ ist eine Abbildung $\{1,..., l\} \to G_1\cup ... \cup G_k$, also eine endliche Folge von ELementen $x_1x_2... x_l$ mit $x_i\in G_1\cup...\cup G_k$ das \emph{leere Wort} bezeichnen wir mit $1$.

Das \emph{Produkt} von zwei Worten ist durch Aneinanderreihen erklärt:
\[
 (x_1x_2...x_l)\cdot (y_1y_2...y_m):=x_1x_2...x_ly_1y_2...y_m
\]
Ein Wort heißt \emph{reduziert}, falls für alle aufeinanderfolgendenden Indexpaare $i, i+1$ mit $x_i\in G_m, x_{i+1}\in G_n$ gilt $m\neq n$. Offensichtlich lässt sich jedes Wort auf kanonische Weise zu einem reduzierten Wort machen ("`\emph{reduzireen}"'), indem man aufeinanderfolgende Elemente, die aus den selben Gruppe $G_i$ stammen, multipliziert, dass heißt durch ihr Produkt ersetzt und neutrale Elemente streicht. 

Wir definieren dann das \emph{freie Produkt} der Gruppen $G_1*... * G_k$ als die Menge der reduzierten Worte
\[
 G_1*...*G_k:=\{x_1...x_l|l\in \N_0, x_1...x_l \text{ reduziertes Wort } \}
\]
...
\[
 \{a^{n_0} b^{n_1} a^{n_2} b^{n_3} a^{n_{2k+1}}|k\in \N_0, n_1 \in \Z, n_1,..., n_{2k}\neq 0\}
\]
...
\]
\[
 (1\cdot 1)\cdot(1\cdot 1)=1\cdot 1 \cdot 1 \cdot 1 = 1\cdot 2 \cdot 1
\]
\[
 (1\cdot 3)\cdot(-3\cdot -1)=1\cdot (3-3)\cdot (-1)=1\cdot 0\cdot (-1)=1\cdot (-1)=0=1=e
\]
wobei $e$ das neutrale Element in $\Z * \Z$
\fixme[from mathematiker : fixen]
\end{note*}

Sei $A\subset G$ eine beliebige Teilmenge der Gruppe $G$. Wir definieren die \emph{von $A$ erzeugte Untergruppe} als die Schnittmenge aller von Untergruppen von $G$, die $A$ enthalten (beliebige Schnitte von Untergruppen sind wieder Untergruppen) und analog den von $A$ erzeugten Normalteiler als die Schnittmenge aller Normalteiler von $G$, die $A$ enthalten (zum Beispiel $G$ ist ein Normalteiler von $G$, der $A$ enthält).
 
\begin{st}[Seifert-van Kampen]\label{thm2:5.2}
 Sei der topologische Raum $X=U\cup V$ mit $U, V\le X$ offen und $U,V, U\cap V$ wegzusammenhängend. Sei $x_0\in U\cap V$. Dann ist
\[
 \pi_1(X;x_0) \cong \pi_1(U;x_0)*\pi_1(V;x_0)/N
\]
wobei $N$ der Normalteiler von $\pi_1(U;x_0)*\pi_1(V_j;x_0)$ ist, der von den Elementen der Form $[\omega]_U\cdot [\omega]_V^{-1}$ mit $\omega\in \Omega(U\cap V; x_0)$ erzeugt wird, wobei die Indizes die Homotopieklassen in $U$ bzw. $V$ (mit festen Basispunkt $x_0$) bezeichnen.
\end{st}
 \fixme[from mathematiker: Überprüfen]


\begin{note*}[Satz von Seifert und van Kampen anschaulich]
 \begin{itemize}
  \item Die $\pi_1(U;x_0)*\pi_1(V;x_0)$ entsteht durch Aneinanderhängen von Schleifen in $U$ bzw. $V$ (mit Basispunkt $x_0$) in beliebiger Reihenfolge
\begin{figure}[H]
\centering
\fixme[fig106]
\caption{}
...
\end{kor}
\begin{proof}
 Es gilt $[\omega]_K=1, [\omega]_V=1$, da alle $\Omega(U\cap V;x_0)$ sogar in $U\cap V$ nullhomotop sind.
\end{proof}
\begin{ex*}
 \begin{enumerate}[(i)]
  \item $\pi_1(S^1\wedge S^1, x_0)\cong \Z*\Z$
\begin{figure}[H]
 \centering
\fixme[fig107]
\caption{}
...
\[
 (g\ker(\phi))\cdot(k\cdot \ker(\phi))
\]
Die Abbildung $\bar \phi$ ist wohldefiniert, denn wenn $k\in \ker(\phi)$, dann ist $\phi(g\cdot k)=\phi(gk)=\phi(g)\cdot \phi(k)=\phi(g)$ und offensicht surjektiv. Sie ist injektiv, denn $\bar \phi(g\cdot \ker(\phi))=1$ impliziert $g\in \ker(\phi)$.

Wir schreiben einen surjektiven Gruppenhomomorpohismus $\phi: \pi_1(U;x_0)*\pi_1(V;x_0)\to \pi_1(X;x_0)$ (und zeigen dann $N=\ker\phi$). (und zeigen dann $N=\ker\phi$).

\emph{Wir definieren $\phi$ wie folgt.} Sei $[\omega_1]_{\omega_1}\cdot [\omega_2]_{\omega_2}\cdot ... [\omega_k]_{\omega_k}$ ein reduziertes Wort in $\pi_1(U; x_0)*\pi_1(V;x_0)$, dass heißt $W_1=U$ oder $V$ aber immer abwechselnd und $[\omega_2]\in \pi_1(W_i;x_0)$.
...
\end{lem}
\begin{proof}
 Sei $\omega\in \Omega(X;x_0)$. Mit einem Kompaktheitsargument kann man zeigen, dass es eine Unterteilung des Intervalls $[0,1]$ gibt, dass heißt $0=t_0<t_1<...<t_{k-1}<t_k=1$, so dass $\omega([t_i, t_{i+1}])$, ganz in $U$ oder ganz in $V$ liegt.
\begin{figure}[H]
 \centering
\fixme[fig109]
\caption{}
\end{figure}
Durch Weglassen von Unterteilungspunkten erreichen wir, dass $U$ und $V$ sich abwechseln, insbesondere gilt dann $\omega(t_i)\in U\cap V$. Wähle Wege $\alpha_i:[0,1]\to U\cap V$ von $x_0$ nach $\omega(t_2).$
...
\begin{align*}
 &[\omega_1]_X\cdot ... \cdot [\omega_k]_X=[\omega_1*...*\omega_k]_X\\
&=\left [\alpha_0*\omega|_{[t_0,t_1]}*\alpha_1*\alpha_1^{-1}*\omega|_{[t_1, t_2]}*\alpha_2*\alpha_2^{-1}...*\alpha_{k-1}*\alpha_{k-1}^{-1}*\omega|_{[t_{k-1},t_k]}*\alpha_k^{-1} \right ]_X\\
&= [\omega|_{[t_1, t_2]}]*...*\omega|_{[t_{k-1}, t_k]}]_X=[\omega]_X.
\end{align*}
\end{proof}
\begin{lem}
 $N\subset \ker(\phi)$.
...
\[
 \{[\omega]_K\cdot [\omega]_V^{-1}|\omega\in \Omega(U\cap V;x_0)\}\subset \pi_1(V;x_0)*\pi_1(V;x_0)\}
\]
...
\end{lem}
\begin{proof}
 Sei $w$ ein Wort $[\alpha_1]\cdot [\beta_1][\alpha_2]\cdot [\beta_2]...[a_k]\cdot [\beta_k]$ mit $[\alpha_i]\in \pi_1(U;x_0)$, $[\beta_i]\in \pi_1(V;x_0)$. Sei $w\in \ker(\phi)$, dass heißt es gibt eine Homotopie 
$H:[0,1]\times[0,1]\to X$, $(s,t)\mapsto H(s,t)$ mit 
\[
 H(\cdot, 0)=\alpha_1*\beta_1*...*\alpha_k*\beta_k, H(\cdot, 2)=x_0
\]
\begin{figure}
 \centering
\fixme[fig110]
...
Die Beweisstragie ist nun, $w=H(\cdot, 0)$, in $x_0=H(\cdot, 1)$ schrittweise "`Zeile für Zeile"' überzuführen, wobei die in jedem Schritt verwendeten Wörter dieselbe Nebenklasse in $\pi_1(U;x_0)*\pi_1(V;x_0)/N$ repräsentieren:
\begin{figure}[H]
 \centering
\fixme[fig111]
\caption{}
...
Sei $f$ eine monoton steigende Funktion $[0,1]\to[0,1]$ die jeweils auf eine Umgebung von $k/n$ konstant ist.
\begin{figure}[H]
 \centering
\fixme[fig112]
\caption{}
...
\end{figure}
\begin{align*}
 [a]_U\cdot [b]_V\cdot [c]_V=[g^{-1}dh]_U\cdot [n^{-1} ek[_V\cdot [k^{-1}f l]_V=[gd]_U\cdot \underbrace{[h]_U[h^{-1}]_V}_{\in N}\cdot [ekk^{-1}fl]_V\stackrel{\text{im Quotiente}}=[g^{-1}]_U[d]_U[e]_V\cdot [f]_V[l]_V 
\end{align*}
\end{ex*}
...
\begin{seg}{Vorbereitungen:}
 Seien $p_0, ..., p_m$ Vektoren im $\R^n$. Eine reelle Linearkombination:
\[
 \lambda_0p_0+\lambda_1p_1+\lambda_2p_2+...+\lambda_mp_m
\]
heißt \emph{Affinkombination}, falls $\lambda_1+\lambda_2+...+\lambda_n=1$ gilt, \emph{Konvexkombination}, falls zusätzlich $0\le \lambda_i\le 1$ für alle $i=0,..., m$ gilt.
...
\caption{}
\end{figure}
Die Menge aller Affinkombinationen bzw. Konvexkombinationen von $p_0,..., p_m$ wird mit $\operatorname{Aff}(p_0,...,p_m)$ bzw. $\operatorname{Kon}(p_0,...,p_m)$ bezeichnet.
 \begin{figure}
 \centering
\fixme[fig118]
\caption{}
\end{figure}
$\operatorname{Kon}(p_0,....,p_m)$ heißt \emph{konvexe Hülle} von $p_0,...,p_m$. Eine Menge $\{p_0,...,p_m\}$ heißt \emph{affinunabhängig}, wenn $\dim(\operatorname{Aff}(p_0,...,p_m))=m$ ist.
\end{seg}
\begin{note*}
 $\operatorname{Aff}(p_0,..., p_m)$ ist affiner Unterraum des $\R^n$
\end{note*}
\begin{exs*}
 \begin{enumerate}[(i)]
...
\end{exs*}
\begin{df}
 Sei $\Sigma\subset \R^n$ ein $m$-dimensionaler Simplex mit Eckpunkten $p_0, ..., p_m$. 
Ein $r$-dimensionales \emph{Teilsimplex} ist eine Teilmenge $\Sigma'\subset \Sigma$ der Gestalt.
\[
 \Sigma'=\Delta(q_0,..., q_r)
\]
wobei $\{q_0,..., q_r\}\subset\{p_0,..., p_m\}$
...
\caption{}
\end{figure}[H]
dass sich je zwei beteiligte Simplizes zur im einem gemeinsamen Teilsimplex schneiden dürfen:
 \begin{figure}
 \centering
\fixme[fig123]
\caption{}
...
\begin{note*}
 \begin{enumerate}[(1)]
  ...
  \item Nach (ii) gilt, dass sich zwei Simplizes zum in einem gemeinsamen Teilsimplex schneiden dürfen. (oder in der leeren Menge)
\end{enumerate}
\end{note*}
Wenn zu den Flächen:
\begin{df}
...
\begin{enumerate}[(i)]
 \item Jedes Simplex von $\mathcal X$ ist einen $2-$Simplex von $\mathcal X$ enthalten.
 \item Jedes $1$-Simplex von $\mathcal X$ ist in höchsten zwei $2$-Simplizes enthalten.
 \begin{figure}
 \centering
\fixme[fig124]
\caption{}
\end{figure}
 \item Ist $p$ ein $0$-Simplex in $\mathcal X$, so gibt es eine Anordnung $\Delta_1,...,\Delta_n$ der $2$-Simplizes
...
\caption{}
\end{figure}
auc: "'\emph{Zerschneidung}"' daran, kombinatorische Fläche mit Rand
 \begin{figure}
 \centering
\fixme[fig127]
\caption{}
...
\begin{enumerate}[(i)]
 ...
 \item die Abbildung $f\vline\,_{\Delta(\sigma)}: \Delta(\sigma)\to \Delta(\sigma)$ affin ist.
\end{enumerate}

Die Abbildung heißt \emph{Isomorphismus}, wenn sie eine simpliziale Umkehrabbildung hat.
\end{df}
\begin{df}
 Zwie kombinatorische Flächen heißen \emph{kombinatorisch äquivalent}, wenn sie isomorphe "`\emph{Unterteilmengen}"' besitzen.
\end{df}
\begin{figure}
 \centering
\fixme[fig129]
...
\begin{df}
 Eine \emph{Zerschneidung} einer kombinatorischen Fläche $(Y,\mathcal Y)$ und einer simplizialen Abbildung. $\phi:(Y, \mathcal Y)\to (X, \mathcal X)$, so dass $\phi\vline\,_{\mathcal Y_2}:\mathcal Y_2 \to \mathcal X_2$ bijektiv ist. (bijektiv auf 2 Simplizes, $0$- und $1$-Simplizes werden eventuell zusammengeklebt.)
\begin{figure}
 \centering
\fixme[fig130]
\caption{}
...
\begin{df}
 Der \emph{Rand} einer kombinatorischen Fläche bestaht aus allen $1$-Simplizes, die nicht Teilimplizes in zwei verschieden $2$-Simplizes sind. Eine kombinatorische Fläche heißt \emph{n-Eck}, wenn ihr Rand aus genau n $1$-Simplizes besteht und sie zu einem $2$-Simplex homöomorph ist.
\begin{figure}
 \centering
\fixme[fig132]
\caption{}
...
\begin{lem}
 Sei $\phi: \mathcal Y\to \mathcal X$ eine Zerschneidung von $\mathcal X$ \emph{entlang eines $1$-Simplex} (dass heißt $\phi^{-1}(e)$ besteht aus zwei $1-$Simplizes von $\mathcal X$, aller anderen haben zur ein $1$-Simplex im Urbild. Sei $\mathcal Y$ die disjunkte Vereinigung eines $n$-Ecks und eines $m$-Ecks. Dann ist $\mathcal X$ ein ($m+n-2$)-Eck.
\end{lem}
\begin{proof}
 \begin{figure}
 \centering
...
\caption{}
\end{figure}
Sei unter all solce eine mit minimale Anzahl von Komponenten gewählt.
\begin{seg}{Diese Anzahl ist gleich 1}
 Da $\mathcal X$ zusammenhängend ist, gibt es nämlich sonst zwei $1$-SImplizes in $Y$, die von $\phi$ identifiziert werden. Nach Lemma \ref{thm3:1.11} lassen sich diese zu einem $n$-Eckk zusammenkleben, wobei eine neue flache Zerschneidung mit verringerter Komponentenzahl entsteht.
\end{seg}
\end{proof}

Eine Zerschneidung $\phi: \mathcal Y \to \mathcal X$ einer geschlossenen Fläche liefert eine Äquivalenzrelation auf dem Rand von $\mathcal Y$: Jede Randkante wird mit genau einer anderen verklebt. Für diese Identifikation gibt es genau zwei Möglichkeiten, die den beiden Möglichkeiten, die den beiden Möglichkeiten entsprechen, die Menge der Endpunkte $\{p_0, p_1\}$ eines $1$-Simplex $\Delta(p_0, p_1)$ auf die Menge der Endpunkte $\{a_0, a_1\}$ eines anderen $1$-Simplex bijektiv abzubilden.

Sei $\mathcal k=\{a_1, ..., a_{2n}\}$ die Menge der Kanten von $\mathcal Y$. Wir gebenden Kanten eine Orientierung, indem wir fordern, dass beim positiven Drucklaufen das Inneren von $\mathcal Y$ links liegen soll.
...
Wir schreiben $a_i^{-1}$ für jeweils die umgekehrt durchgelaufene Kante. Wir setzen $\mathcal k^0:=\{a_1,..., a_{2n}, a_1^{-1}, ..., a_{2n}^{-1}\}$ für die Menge der \emph{orienterten Kante}.
...
\[
 a_{i_1}^{\epsilon_i} a_{i_2}^{\epsilon_2}.... a_{i_{2r}}^{\epsilon_{2r}}
\]
wobei $\epsilon_0=\pm 1$ und jedes Index $i\in \{1,..., r\}$ unter der Indizes $i_1, i_2, ..., i_{2r}$ genau zweimal vorkommt. Das zugehörige Polygonmodell besteht aus einem $2r$-Eck mit zyklisch angeordneten Kanten $b_1,..., b_{2r}$ mit
\[
 \psi(b_\mu)=b_v^\epsilon \text{ für } i_\mu=i_v \text{ und } \epsilon_\mu=\epsilon \epsilon_0
\]
...
\begin{enumerate}
 ...
 \item $...a^{\epsilon}...a^{\epsilon'}... \sim ...a^{-\epsilon}...a^{-\epsilon'}...$
 \item $...ab... ab... \sim ... a ... a...$
 \item $...ab... b^{-1} a^{-1}... \sim ... a ... a^{-1} ...$
...
\caption{}
\end{figure}
$...c...c...$ auch \emph{Kreuzhaube} genannt.
\end{note*}
...
\end{ex*}
\begin{proof}
 Die Ecke $P$ kommt nun in verringerter Anzahl vor ($Q$ dafür einmal öfter). Durch Wiederholen erreicht man, dass $P$ nur noch einmal vorkommt. Dann haben wir entweder $aa^{-1}$ (fertig) oder $aa^{-1}...$ und wir können "`Beiziehen"'.
\end{proof}

\begin{lem}[Kreuzhauben-Normierung]
 Ein gekürztes Flächen-Wort mit Eckenzahl $1$ und $k$ Kreuzhauben ist äquivalent zu $c_1c_1c_2c_2...c_kc_k...$, wobei alle sonstigen Kanten genau einmal mit Exponent $+1$ und einmal mit $-1$ auftreten.
\end{lem}
\begin{proof}
 \begin{figure}
 \centering
...
\caption{}
\end{figure}
Bereits vorhandene Kreuzhauben der Form $dd$ werden dabei nicht vereinfacht.
\end{proof}

\begin{lem}[Henkel-Normierung]
 Ein gekürztes Flächen-Wort mit Eckenzahl $1$ ist äquivalent zu einem Produkt von Worten der Gestalt $c_ic_j$ und $a_jb_ja_j^{-1}b_j^{-1}$
\end{lem}
\begin{proof}
 Wir nehmen an, dass die Kreuzhauben bereits normiert wie in Lemma \ref{thm3: 1.18} vorkommen. Dann muss das Wort von der Form $...a...b...a^{-1}...b^{\pm1}...$, denn sonst würde der Rand des Polygons, wenn man $a$ und $a^{-1}$ entfernt (nach Identifikation der übrigen entsprechenden Kanten) in zwei Komponenten zerfallen,
\begin{figure}
 \centering
\fixme[fig148]
\caption{}
\end{figure}
Widerspruch zu Eckenzahl $1$. Der Exponent von $b^{\pm 1}$ muss $-1$ sein, wegen der vorherigen Kreuzhauben-Normierung. Wir formen nun $...a...b...a^{-1}...b^{-1}...$ zu $...aba^{-1}b^{-1}...$
\begin{figure}
 \centering
\fixme[fig149]
\caption{}
...
\begin{lem}[Henkel-Elimination]
 Ein gekürztes Flächenwort mit Eckenzahl $1$, dass mindestens eine Kreuzhaube enthält, ist äquivalent zu einem Wort der Gestalt $c_1c_1c_2c_2...c_kc_k$
(einem Produkt von normierten Kreuzhauben).
\end{lem}
\begin{proof}
 Falls außer einer Kreuzhaube noch mindestens ein Henkel vorkommt, ist das Wort äquivalent zu $ccaba^{-1}b^{-1}...$ Wir formulieren:
\begin{figure}
 \centering
\fixme[fig150]
\caption{}
...
\begin{note*}[Endergebnis]
 ...
 \begin{enumerate}[(1)]
  \item $aa^{-1}$ die Rechenweg dieses Worts  (die Sphäre) nennen wir auch $F_0$
  \item $a_1b_1a_1^{-1} b_1^{-1} a_2 b_2a_2^{-1} b_2^{-1}...a_gb_g a_g^{-1} b_g^{-1}$ der Realisierung nennen wir auch $F_g, g\ge 1$
  \item $c_1c_1c_2c_2...c_g$ die Realisierung nennen wir auch $N_g$.
 \end{enumerate}
Die Fläche $F_g, g\ge 0$ nennt man die \emph{orientierbare geschlossenen Flächen von Geschlecht} $g$.
\begin{figure}
 \centering
\fixme[fig151]
\caption{}
\end{figure}
Die Flächen $N_g, g\ge 1$ ennt man die \emph{nicht-orientierbaren Flächen} vom Geschlecht $g$
\begin{figure}
 \centering
\fixme[fig152]
\caption{}
...
\end{st}
\begin{st}
 Sei $\omega(a_1,...,a_n)=a^{\epsilon_1}_{i_1} a^{\epsilon_2}_{i_2}... a^{\epsilon_2r}_{i_{2r}}$ ein FLächenwort mit Eckenzahl $1$. Sei $X$ die Realisierung des zugehörigen Polygon-Modells und $x_0\in X$ das gemeinsame Bild der Endpunkte aller Kanten, dann hat $\pi_1(X;x_0)$ die Präsentation
\[
 \pi_1(X;x_0)=\langle a_1,..,a_r \, \vline \, w(a_1,...a_r)=1\rangle
\]
dass heißt die freie Gruppe über $a_1,..., a_r$ modulo dem von $w(a_1,..., a_r)$ erzeugten Normalteiler.
\begin{kor*}
 $\pi_1(F_g)_{ab}\cong \Z^g$, $\pi_1(N_0)_{ab}\cong \Z^{g-1} \oplus \Z_2$ abelsch gemachte Gruppe
\end{kor*}
