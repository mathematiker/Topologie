...
\begin{note*}
 \begin{itemize}
  \fixme[from GnuLinux : S kann so nicht stimmen ????? ]
  \fixme[from mathematiker : Ich konnte das nicht ganz überblicken?]
  \fixme[from mathematiker : wofür ist die Bemerkung gut?]
  \fixme[from GnuLinux: dass $\R^3$ in der Bemerkung ist finde ich gut, es ist zwar nicht notwendig ist aber eine interessante Betrachtungsweise, die Bemerkung zu $\mathcal S$ ist leider formal immer noch falsch, z.B. ist im allgemeinen ja $X_k \notin \mathcal S$, sondern $ \dotsb \times X_k \times \dotsb \in \mathcal S$. Auch kann man das nicht so einfach mit Tupelschreibweise oder ähnlichem retten, da die Komponenten der Tupel ja Elemente !!! aus $X_k$ und nicht die Räume $X_k$ oder offenen Mengen $U_k \subset X_k$ sind.
Wenn dann müßte hier sowas wie
 $\mathcal S = \{X_1 \times X_2 \times \dotsb \times X_{k-1} \times U \times X_{k+1}, \dotsb \times X_n | U\subset X_k \texttt { offen }, k \in \{1, \dotsc n \}\}$,
 dies wäre aber erstens nur für endliche Produkte so, zweitens steht oben bereits dasselbe, nur viel besser aufgeschrieben da, und ich habe diese Bemerkung in meinem eigenen handschriftlichen Aufschrieb überhaupt nicht, ich würde das aus der Bemerkung einfach entfernen. 
melde dich mal bei mir abdelaar@studi.informatik.uni-stuttgart.de, das problem ist ich habe versucht dir eine nachricht über github zu schicken bin aber zu doof dazu und möchte auch nicht genauer recherchieren wie das genau funktioniert]
\fixme[jetzt hat sichs geklärt, dass habe ich eigentlich mit der Notation gemeint... Du hast natürlich recht... Tatsächlich soll dies exemplarisch für endliche Produkte sein... Sonst ist mir der Rest was du bemängelst unklar]
  \item $\mathcal S=\{X_1\times X_2\times ... \times X_{k-1} \times U \times X_{k+1} \times...\times X_n$ mit $U\subset X_k$ offen $, k \in \{1, \dotsc n \} \} $
...
\end{st}
\begin{proof}
 Sei $X$ ein Hausdorffraum und $A\subset X$ kompakt mit der induzierten Topologie. Wir wollen zeigen, dass es um jeden Punkt $x\in X\setminus A$ eine Umgebung gibt,
 die A nicht trifft, dann ist $X\setminus A$ offen. Sei $p\in X\setminus A$. Nach der Hausdorffeigenschaft von $X$ finden wir zu jedem $a\in A$ eine offene Umgebung $U_a$ von $a$ und eine 
offene Umgebung $V_a$ von $p$, so dass $U_a\cap V_a=\emptyset$ gilt. \\
\begin{figure}[h]
...
\end{df}
\begin{ex*}
\fixme[from mathematiker : Ich hoffe es passt so jetzt... Also er muss es in der Vorlesung erwähnt haben, sonst hätte ich das nicht aufgeschrieben...  Also jetzt mal ganz informell:  Wenn du ne Menge über die Folgenglieder einer Folge definierst (also auch für beliebig , so genügt die Beschränktheit als Kriterium für die Kompaktheit... Überleg dir ne gescheite Formulierung und dann hamma glaub ich das Problem aus der Welt]
